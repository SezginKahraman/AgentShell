...
func registerCatalogTools(server *mcp.Server, client *daemonClient) {
	addTool(server, "get_workspace_context", "Get workspace context", toolIntent+"Read the explicitly configured MCP workspace root and the workspace environment names. This never infers a path from the daemon working directory and executes nothing. Use those environment names with start_stack instead of cloning a stack per profile.", readOnly("Get workspace context"), nil,
		func(ctx context.Context, _ EmptyInput) (map[string]any, error) {
			...
			if client.config.baseURL != nil {
				if lib, err := client.do(ctx, http.MethodGet, "/api/environments?redact_secrets=1", nil, nil); err == nil {
	...
	addTool(server, "list_environments", "List environments", toolIntent+"Read the workspace environment library: named columns such as local and prod, keys defined once, and their values. Keys in secret_keys appear as *** so tokens stay out of chat; the key names remain for {{KEY}} interpolation.", readOnly("List environments"), nil,
		func(ctx context.Context, _ EmptyInput) (map[string]any, error) {
			return client.do(ctx, http.MethodGet, "/api/environments?redact_secrets=1", nil, nil)
		})

	addTool(server, "update_environments", "Update environments", toolIntent+"Replace the workspace environment library. Keys are defined once; names are columns. custom is reserved. Deleting a name remaps stacks that used it. secret_keys marks tokens. A secret cell of *** keeps the stored value. Do not send real secret values; humans set those in Settings.", mutating("Update environments", false, false), UpdateEnvironmentsInput.validate,
	...
	addTool(server, "save_http_request", "Save HTTP request", toolIntent+"Add an independent HTTP request to an HTTP collection. URL, headers, and body may use {{KEY}} from the workspace environment library. Optional named body_templates; body is the active template used by send. If method, URL, and headers already match a saved request and only the body differs (for example another hotel id), do not create a second request: add a named body_templates entry with update_http_request. Never store secrets. This does not send the request and is not save_check.", mutating("Save HTTP request", false, false), SaveHTTPRequestInput.validate,
	...
	addTool(server, "update_http_request", "Update HTTP request", toolIntent+"Update a saved HTTP request without sending it. If only the body differs from an existing case (same method, URL, and headers; for example another hotel id), add a named body_templates entry: send the full template list plus the new one, set active_body_id, and set body to that template. Do not save_http_request a duplicate. Keep secrets out of URL, headers, and body.", mutating("Update HTTP request", false, false), UpdateHTTPRequestInput.validate,
