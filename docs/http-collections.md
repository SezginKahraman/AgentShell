...
Method, URL, headers, body, timeout. The request name is a title above the URL and saves on blur, like the collection name; it does not wait for body Save. One request can keep several named **body templates**. The editor body is what **Send** and curl use. **Send** does not write templates; it only stores `last_result`. **New** copies the current body as a draft you can rename. **Save** keeps it. **Delete** drops the selected body. Switching among already saved bodies persists `active_body_id` immediately and stays Saved. Unsaved edits still warn before leaving the request. **Beautify** pretty-prints the active JSON or XML body in the editor; unquoted `{{KEY}}` placeholders and other non-parseable text stay unchanged until Save. Unsaved drafts stay in the editor; refreshing the page warns, then discards them. URL, headers, and body may use `{{KEY}}` from the workspace library (and stack extras when bound). Put tokens in the library and mark them `secret_keys`; keep `{{GOOGLE_TOKEN}}` in the request, never the raw value.
...
`POST /api/http-requests/{id}/send` interpolates, sends, and stores `last_result` on the request (status, headers, body). An optional JSON overlay (`method`, `url`, `headers`, `body`, `timeout_ms`) is used only for that send; it does not update saved templates. That is not a process Run. Secret library values are interpolated for the wire request, then replaced with `***` in persisted `last_result` (URL, headers, body, error) and in the dashboard interpolated curl/preview so Copy request is paste-safe. Chips may still show the Settings value. The dashboard shows that result in a compact Logs-style terminal (traffic lights, status chips, headers, JSON body). The pane also shows the interpolated request as curl; multiline curl starts collapsed and expands on click. Copy actions cover that curl, the full response, and the response body (a left-aligned Copy body control, so a wide payload does not hide it). **Beautify** next to Copy body pretty-prints XML (and JSON that was left compact); parseable JSON is already pretty-printed on display. While a send is in flight the pane replaces the previous body with a waiting state.
